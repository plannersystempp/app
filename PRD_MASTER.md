...
## [2026-03-05] - fix: Validação de datas no formulário de eventos - Melhorias de UX
 ...
 - Arquivos:
   - `src/components/events/EventForm.tsx`
 - Impacto:
   - Usuários agora recebem feedback visual imediato sobre erros de validação
...
## [2026-03-06] - fix: Correção de scroll em lista de pessoal e melhoria na busca
 - Mudanças:
   - Revertida a implementação de `useMomentumScroll` que estava bloqueando o scroll nativo da roda do mouse em `PersonnelSelector`.
   - Adicionada classe `scroll-smooth` ao `CommandList` para garantir suavidade nativa via CSS.
   - Adicionada classe `pointer-events-auto` ao `PopoverContent` para garantir que eventos de mouse não sejam bloqueados.
   - Implementada função de filtro customizada no componente `Command` (`filterFunction`) que ignora acentos (normalização NFD) e case sensitive, permitindo que buscas como "joao" encontrem "João".
 - Arquivos:
   - `src/components/events/allocation/PersonnelSelector.tsx`
   - `src/hooks/useMomentumScroll.ts` (Deprecado/Removido do fluxo)
 - Impacto:
   - Restaura o funcionamento do scroll via roda do mouse.
   - Melhora significativamente a experiência de busca, permitindo encontrar nomes com acentos digitando sem acentos.
