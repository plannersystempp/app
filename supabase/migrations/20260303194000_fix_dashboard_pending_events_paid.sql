-- Fix: Dashboard ainda lista eventos como "pendente" mesmo quando pagos
-- Alinha o cálculo de `has_pending_payments` da RPC `get_events_with_payment_status` com a lógica do frontend:
-- - Usa cachê por evento/função (event_specific_cache/custom_cache/event_cache)
-- - Usa taxa de hora extra por função (custom_overtime) e regra max(cache/12)
-- - Considera conversão diária de horas extras (threshold + cobertura de 8h)
-- - Desconta faltas (work_records.attendance_status='absent') dos dias trabalhados
...
BEGIN
    ...
    SELECT
      ...
      MAX(pf.custom_cache) FILTER (
        WHERE COALESCE(pf.custom_cache, 0) > 0
          AND (
            (pa.function_id IS NOT NULL AND pf.function_id = pa.function_id)
            OR (pa.function_name IS NOT NULL AND f.name = pa.function_name)
          )
      ) AS max_custom_cache,
  ...
  absent_days AS (
    SELECT
      ...
      ARRAY_AGG(DISTINCT wr.work_date) FILTER (WHERE wr.work_date IS NOT NULL) AS absent_days
    ...
    SELECT
      ...
      (
        ...
        COALESCE((
          ...
          WHERE pc.event_id::TEXT = ANY(pp.related_events)
  ...
  event_agg AS (
    SELECT
      ...
      COUNT(DISTINCT CASE
        WHEN pep.paid > 0 AND pep.paid >= pep.expected - 0.01 THEN pep.personnel_id
        ELSE NULL
      END)::INT AS paid_count,
      COALESCE(BOOL_OR(pep.expected > pep.paid + 0.01), FALSE) AS has_pending_payments
