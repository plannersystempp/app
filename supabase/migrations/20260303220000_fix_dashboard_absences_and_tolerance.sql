-- Fix: Eventos pagos ainda aparecem no Dashboard devido a faltas em nível de alocação e tolerância de arredondamento.
-- Alinha get_events_with_payment_status com as regras de calculateUniqueWorkDays do frontend.
...
BEGIN
  ...
  -- Filtra pessoas alocadas que não são fixas e não estão marcadas como falta total na alocação
  alloc_people AS (
    SELECT DISTINCT
      pa.event_id,
      pa.personnel_id
    ...
    WHERE pa.team_id = p_team_id
      ...
      AND COALESCE(pa.attendance_status, 'pending') <> 'absent' -- REGRA: Ignora alocação se status for 'absent'
  ),
  alloc_rates AS (
    SELECT
      pa.event_id,
  ...
  event_agg AS (
    SELECT
      ...
      COUNT(DISTINCT CASE
        WHEN pep.paid >= pep.expected - 0.10 THEN pep.personnel_id -- Tolerância de 10 centavos
        ELSE NULL
