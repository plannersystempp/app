-- Fix dashboard pending logic to respect payroll closings
-- If a person has a payroll closing record, they are considered settled/paid for that event.
-- Also fixes reference to non-existent column pa.function_id (uses function_name instead).
...
BEGIN
    ...
    SELECT
      ...
      MAX(pf.custom_cache) FILTER (
        WHERE COALESCE(pf.custom_cache, 0) > 0
          AND (
            -- Removed pa.function_id check as column does not exist
  ...
  absent_days AS (
    SELECT
      ...
      -- FIX: Cast work_date to TEXT to match work_days type (TEXT[])
  ...
  person_expected_paid AS (
    SELECT
      ...
      EXISTS (
        SELECT 1 FROM public.payroll_closings pc2
        WHERE pc2.event_id = pc.event_id
          AND pc2.personnel_id = pc.personnel_id
      ) AS is_closed
    FROM person_calc pc
  ...
  event_agg AS (
    SELECT
      ...
      COUNT(DISTINCT CASE
        WHEN pep.is_closed OR (pep.paid >= pep.expected - 0.10) THEN pep.personnel_id
        ELSE NULL
      END)::INT AS paid_count,
      COALESCE(BOOL_OR(
        NOT pep.is_closed AND (pep.expected > pep.paid + 0.10)
      ), FALSE) AS has_pending_payments
