-- v6: Fix 400 error and improve partial payment detection
-- Remove referência a pc.total_amount (que não existe) e replica a lógica de cálculo do frontend
...
BEGIN
    ...
    WITH person_totals AS (
        ...
        SELECT 
            pa.event_id,
            pa.personnel_id,
            -- Esperado: (Dias alocados * Cache) + (Soma de Horas Extras * Taxa)
            (
                (COALESCE(cardinality(pa.work_days), 0) * COALESCE(pa.event_specific_cache, p.event_cache, 0))
                +
                COALESCE((
    ...
    event_aggregation AS (
        SELECT 
            ...
            COUNT(DISTINCT CASE WHEN pt.paid >= pt.expected - 0.10 AND pt.expected > 0 THEN pt.personnel_id END)::INT as total_paid,
