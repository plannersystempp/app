...
-- Ajusta o RPC do dashboard para considerar `payroll_closings` como quitação definitiva por pessoa no evento.
-- Regra: se existe closing para (event_id, personnel_id), não deve reabrir pendência por recálculo de taxa.
...
BEGIN
  ...
  person_expected_paid AS (
    SELECT
      ...
      EXISTS (
        SELECT 1
        FROM public.payroll_closings pcx
        WHERE pcx.event_id = pc.event_id
          AND pcx.personnel_id = pc.personnel_id
      ) AS has_closing
    FROM person_calc pc
    CROSS JOIN team_cfg tc
  ),
  event_agg AS (
    SELECT
      ...
      COUNT(DISTINCT CASE
        WHEN pep.has_closing OR pep.paid >= pep.expected - 0.10 THEN pep.personnel_id
        ELSE NULL
      END)::INT AS paid_count,
      COALESCE(BOOL_OR((NOT pep.has_closing) AND (pep.expected > pep.paid + 0.10)), FALSE) AS has_pending_payments
